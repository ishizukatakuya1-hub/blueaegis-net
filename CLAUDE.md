...
**2026-09-25 ユーザー決定：毎朝7時の自動実行（手順書はリポジトリ外の `../blueaegis-net_private/docs/DAILY.md`）は、下の3つのゲートをすべて通った記事に限り、人の確認なしで公開してよい。**
1つでも通らなければ `draft: true` のまま残し、理由を通知する。

1. `node tools/build.js` が成功する（広告表示・rel="sponsored"・PR・一次出典・`【要確認】` の検査を含む）。
2. 事実確認：本文の数値・日付・条番号・料金・料率・締切を**すべて**一次資料で再確認する。料金・料率・締切は、お知らせ一覧や更新日を見て**最新版であること**まで確認する（2026-09-24 に、お名前.com の調整費で古いお知らせを最新と取り違えた）。確認できないものが1件でもあれば不合格。
3. 公開前点検：ステマ・優良誤認・有利誤認、特許の「解決の仕組み」を書いていないか、未出願分野に触れていないか。

上限は1日1本、同じ広告主の記事は週2本まで。記事の比較・評価は報酬の有無で変えない。
手動で書く記事は従来どおり、ユーザーが本文を確認してから公開する。
